...
// if-setning, teller feilmeldinger
  ...
  if(empty($brukernavn)) {
    echo "Brukernavn er påkrevd<br>";
    $tellFeil = 1;
  }
  if(empty($epost)) {
    echo "E-post er påkrevd<br>";
    $tellFeil = 1;
  }
  if(empty($fornavn)) {
    echo "Fornavn er påkrevd<br>";
    $tellFeil = 1;
  }
  if(empty($etternavn)) {
    echo "Etternavn er påkrevd<br>";
    $tellFeil = 1;
  }
  if(empty($passord_1)) {
    echo "Lag et passord er påkrevd<br>";
    $tellFeil = 1;
  }
  if(empty($passord_2)) {
    echo "Bekreft passord er påkrevd<br>";
    $tellFeil = 1;
  }
  // er passordene ulike?
  if($passord_1 != $passord_2) {
    echo "Passordene må være lik<br>";
    $tellFeil = 1;
  }
  // er eposten gyldig?
...
if($tellFeil < 1) {
  ...
  if($kobling->query($sql_registrerBruker)) {
    echo "Brukeren er registrert! Du kan: <a href='logginn.php'>Logge inn</a>";
  } else {
